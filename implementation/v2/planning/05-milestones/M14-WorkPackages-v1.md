# M14 Work Packages — Budget Persistence, Real Cost Model, and Grafana Dashboard

Milestone: `M14`
Status: `planned`
Entry gate: M13 complete (LangGraph active, project spaces wired, PostgreSQL stable)
...
## WP M14-01 — PostgreSQL Budget Ledger
...
## WP M14-02 — Token-Based Cost Model
...
**Entry criteria:** WP M14-01 complete (budget ledger and `settle()` method available).

### Tasks

- [ ] Implement `src/openqilin/budget_runtime/cost_evaluator.py` — `TokenCostEvaluator`:
  - `estimate(model_class, estimated_input_tokens)` — returns `CostEstimate` using `COST_PER_1K_TOKENS` lookup; rough output multiplier 2×
  - `settle(response_metadata)` — returns `ActualCost` using `response_metadata.total_tokens` and `response_metadata.cost_usd`
  - `COST_PER_1K_TOKENS = {"gemini_flash_free": 0.0, "gemini_flash": 0.000035, "gemini_pro": 0.00125}`
- [ ] Update `LlmGatewayService` to call `TokenCostEvaluator.estimate()` before dispatch and `settle()` after LLM response is received; call `budget_client.settle()` with actual cost
- [ ] Remove the character-count cost formula from all code paths
- [ ] Add unit tests:
  - `estimate()` returns `CostEstimate` with correct USD for known model class
  - `settle()` uses `response_metadata.total_tokens`, not character count
  - Unknown model class uses default rate `0.001`

### Outputs

- Real token-based cost model active for all LLM calls
...
## WP M14-03 — Budget Obligation Enforcement
...
**Entry criteria:** WP M12-02 (obligation dispatcher active), WP M14-01 (budget ledger active).
...
## WP M14-04 — Bug Fixes: M-4 and M-5
...
**Entry criteria:** WP M14-01 complete (budget client is now always non-None in production; M-4 is defensive coding).
...
## WP M14-05 — Grafana Dashboard Build
...
**Entry criteria:** M12 OTel and PostgreSQL data sources connected to Grafana; M13 project spaces active (data available for project panels); M14-01 budget data available.
...
## WP M14-06 — Grafana Alerting and Discord Webhook
...
**Entry criteria:** WP M14-05 complete (dashboard with panels); Discord webhook URL available.
