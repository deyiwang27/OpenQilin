"""Static checks for M9 live acceptance artifact completeness."""
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate deterministic M9 live acceptance artifact set.",
    )
    parser.add_argument(
        "--repo-root",
        type=Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    preflight_report = (
        repo_root / "implementation/v1/planning/artifacts/m9_live_preflight_latest.json"
    )
    manifest = (
        repo_root / "implementation/v1/planning/artifacts/m9_live_acceptance_manifest_latest.json"
    )
    notes = repo_root / "implementation/v1/planning/artifacts/m9_live_acceptance_notes.md"

    for artifact in (preflight_report, manifest, notes):
        if not artifact.exists():
            failures.append(f"missing required M9 artifact: {artifact.relative_to(repo_root)}")
    ...
    if manifest.exists():
        ...
        required_scenarios = {"completed_archive_branch", "terminated_archive_branch"}
        if scenario_names != required_scenarios:
            failures.append(
                "manifest scenarios mismatch; expected completed_archive_branch + terminated_archive_branch"
            )
    ...
    if failures:
        print("M9 live acceptance artifact checks failed:")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("M9 live acceptance artifact checks passed.")
    return 0
