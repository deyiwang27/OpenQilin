"""M18-WP1 unit tests for Administrator conversational advisory."""
...
def _request() -> FreeTextAdvisoryRequest:
    return FreeTextAdvisoryRequest(
        text="How do we quarantine a misbehaving agent?",
...
def _make_agent(
    *,
    ...
    return AdministratorAgent(
        document_policy=MagicMock(),
        retention=MagicMock(),
        governance_repo=MagicMock(),
        agent_registry_repo=MagicMock(),
...
def test_handle_free_text_uses_history_persists_turns_and_increments_metric() -> None:
    llm = MagicMock()
    llm.complete.return_value = _served_response("Administrator advisory")
    ...
    assert response.advisory_text == "Administrator advisory"
    conversation_store.append_turns.assert_called_once_with(
        "guild::guild-1::channel::channel-1",
        user_prompt="How do we quarantine a misbehaving agent?",
        assistant_reply="Administrator advisory",
        agent_id="administrator",
    )
    prompt = llm.complete.call_args.args[0].messages_or_prompt
    assert "Conversation so far:" in prompt
    assert (
        recorder.get_counter_value("llm_calls_total", labels={"purpose": "administrator_response"})
        == 1
    )


def test_handle_free_text_returns_fallback_when_llm_gateway_is_missing() -> None:
    agent = _make_agent()

    response = agent.handle_free_text(_request())

    assert response.advisory_text.startswith("I'm the Administrator agent.")
    assert "/oq ask administrator <topic>" in response.advisory_text


def test_handle_free_text_continues_when_store_read_fails() -> None:
    llm = MagicMock()
    llm.complete.return_value = _served_response("Administrator advisory after read failure")
    conversation_store = MagicMock()
    ...
    assert response.advisory_text == "Administrator advisory after read failure"
    conversation_store.append_turns.assert_called_once()


def test_handle_free_text_continues_when_store_write_fails() -> None:
    llm = MagicMock()
    llm.complete.return_value = _served_response("Administrator advisory after write failure")
    conversation_store = MagicMock()
    conversation_store.list_turns.return_value = ()
    ...
    assert response.advisory_text == "Administrator advisory after write failure"
