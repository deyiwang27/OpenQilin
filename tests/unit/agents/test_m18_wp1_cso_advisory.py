"""M18-WP1 unit tests for CSO conversational advisory."""
...
def _request() -> FreeTextAdvisoryRequest:
    return FreeTextAdvisoryRequest(
        text="What strategic conflicts should we watch for?",
...
def test_handle_free_text_uses_history_persists_turns_and_increments_metric() -> None:
    llm = MagicMock()
    llm.complete.return_value = _served_response("CSO advisory")
    ...
    assert response.advisory_text == "CSO advisory"
    conversation_store.append_turns.assert_called_once_with(
        "guild::guild-1::channel::channel-1",
        user_prompt="What strategic conflicts should we watch for?",
        assistant_reply="CSO advisory",
        agent_id="cso",
    )
    prompt = llm.complete.call_args.args[0].messages_or_prompt
    assert "Conversation so far:" in prompt
    assert "assistant: previous answer" in prompt
    assert recorder.get_counter_value("llm_calls_total", labels={"purpose": "cso_response"}) == 1
...
def test_handle_free_text_returns_fallback_on_denied_llm_response() -> None:
    ...
    assert response.advisory_text.startswith("I'm the CSO agent.")
    assert "/oq ask cso <topic>" in response.advisory_text


def test_handle_free_text_continues_when_store_read_fails() -> None:
    llm = MagicMock()
    llm.complete.return_value = _served_response("CSO advisory after read failure")
    conversation_store = MagicMock()
    ...
    assert response.advisory_text == "CSO advisory after read failure"
    conversation_store.append_turns.assert_called_once()


def test_handle_free_text_continues_when_store_write_fails() -> None:
    llm = MagicMock()
    llm.complete.return_value = _served_response("CSO advisory after write failure")
    conversation_store = MagicMock()
    conversation_store.list_turns.return_value = ()
    ...
    assert response.advisory_text == "CSO advisory after write failure"
