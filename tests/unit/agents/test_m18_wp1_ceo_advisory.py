"""M18-WP1 unit tests for CEO conversational advisory."""
...
def _request() -> FreeTextAdvisoryRequest:
    return FreeTextAdvisoryRequest(
        text="What should we prioritize at the executive level?",
...
def test_handle_free_text_uses_history_persists_turns_and_increments_metric() -> None:
    llm = MagicMock()
    llm.complete.return_value = _served_response("CEO advisory")
    ...
    assert response.advisory_text == "CEO advisory"
    conversation_store.list_turns.assert_called_once_with("guild::guild-1::channel::channel-1")
    conversation_store.append_turns.assert_called_once_with(
        "guild::guild-1::channel::channel-1",
        user_prompt="What should we prioritize at the executive level?",
        assistant_reply="CEO advisory",
        agent_id="ceo",
    )
    ...
    assert recorder.get_counter_value("llm_calls_total", labels={"purpose": "ceo_response"}) == 1
...
def test_handle_free_text_returns_fallback_on_denied_llm_response() -> None:
    ...
    assert response.advisory_text.startswith("I'm the CEO agent.")
    assert "/oq ask ceo <topic>" in response.advisory_text


def test_handle_free_text_continues_when_store_read_fails() -> None:
    llm = MagicMock()
    llm.complete.return_value = _served_response("CEO advisory after read failure")
    conversation_store = MagicMock()
    ...
    assert response.advisory_text == "CEO advisory after read failure"
    conversation_store.append_turns.assert_called_once()


def test_handle_free_text_continues_when_store_write_fails() -> None:
    llm = MagicMock()
    llm.complete.return_value = _served_response("CEO advisory after write failure")
    conversation_store = MagicMock()
    conversation_store.list_turns.return_value = ()
    ...
    assert response.advisory_text == "CEO advisory after write failure"
