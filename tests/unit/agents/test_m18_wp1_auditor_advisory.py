"""M18-WP1 unit tests for Auditor conversational advisory."""
...
def _request() -> FreeTextAdvisoryRequest:
    return FreeTextAdvisoryRequest(
        text="What governance violations should trigger escalation?",
        scope="guild::guild-1::channel::channel-1",
        guild_id="guild-1",
        channel_id="channel-1",
        addressed_agent="auditor",
    )
...
def test_handle_free_text_uses_history_persists_turns_and_increments_metric() -> None:
    llm = MagicMock()
    llm.complete.return_value = _served_response("Auditor advisory")
    ...
    assert response.advisory_text == "Auditor advisory"
    conversation_store.append_turns.assert_called_once_with(
        "guild::guild-1::channel::channel-1",
        user_prompt="What governance violations should trigger escalation?",
        assistant_reply="Auditor advisory",
        agent_id="auditor",
    )
    prompt = llm.complete.call_args.args[0].messages_or_prompt
    assert "Conversation so far:" in prompt
    assert (
        recorder.get_counter_value("llm_calls_total", labels={"purpose": "auditor_response"}) == 1
    )
...
def test_handle_free_text_returns_fallback_when_llm_gateway_is_missing() -> None:
    ...
    assert response.advisory_text.startswith("I'm the Auditor agent.")
    assert "/oq ask auditor <topic>" in response.advisory_text


def test_handle_free_text_continues_when_store_read_fails() -> None:
    llm = MagicMock()
    llm.complete.return_value = _served_response("Auditor advisory after read failure")
    conversation_store = MagicMock()
    ...
    assert response.advisory_text == "Auditor advisory after read failure"
    conversation_store.append_turns.assert_called_once()


def test_handle_free_text_continues_when_store_write_fails() -> None:
    llm = MagicMock()
    llm.complete.return_value = _served_response("Auditor advisory after write failure")
    conversation_store = MagicMock()
    conversation_store.list_turns.return_value = ()
    ...
    assert response.advisory_text == "Auditor advisory after write failure"
